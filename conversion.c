...
char* BinaryString (bool isNegative, int len, long int num, bool isThirtyTwo) {
	...
	free (revBin);
	if (isThirtyTwo == 1) bin[0] = '1';                               // In case of 32 bit negative numbers, 2^31 is reduced in Binary() hence to compensate, 32nd bit is turned to 1
	return bin;
}
...
char* Binary (long int num) {
	...
	if (num < 0) {
		...
		for (digit = 0; numCpy > 0; digit++)  numCpy >>= 1;
...
/// <summary>Method to print hexadecimal numbers using format specifier.</summary>
void Hex (int num) {
	printf ("HEX : %X", num);
}
